...
const fallback = [
  ...
  { id: 4, name: 'Trần Văn Minh', school: 'Du học Nhật Bản', quote: 'Nhờ HCIT em biết cách chuẩn bị hồ sơ đúng chuẩn, tiết kiệm được rất nhiều thời gian và tiền bạc so với tự làm. Hiện tại em đang học tại Tokyo và rất hài lòng với cuộc sống ở đây. Cảm ơn anh chị đã hỗ trợ em từ khâu chọn trường cho đến khi cầm visa trên tay.', avatar: 'https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?w=150&q=80' },
  ...
  { id: 7, name: 'Lê Thị Mai', school: 'Du học Úc', quote: 'Ban đầu tôi rất lo lắng vì điều kiện tài chính hạn chế, nhưng HCIT đã tư vấn cho tôi các phương án học bổng phù hợp và lộ trình tiết kiệm chi phí. Giờ tôi đang học ngành Y tế cộng đồng tại Melbourne với học bổng 40% từ trường.', avatar: 'https://images.unsplash.com/photo-1438761681033-6461ffad8d80?w=150&q=80' },
...
export default function Testimonials() {
  ...
  // Lấy 3 card liên tiếp từ vị trí current
    ...
    <section className="py-20 bg-gradient-to-b from-white to-gray-50">
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="relative">
          ...
          <button
            onClick={() => go(-1)}
            className="absolute -left-4 top-1/2 -translate-y-1/2 z-10 w-10 h-10 bg-white border border-gray-200 rounded-full shadow-md flex items-center justify-center hover:bg-orange-50 hover:border-orange-300 transition-colors"
            aria-label="Trước"
          >
          ...
          <div className="grid md:grid-cols-3 gap-6 overflow-hidden">
            {visible.map((t, i) => (
              <div
                key={`${t.id}-${i}`}
                className="rounded-2xl p-6 bg-white border border-gray-100 shadow-sm hover:shadow-md transition-all duration-300 flex flex-col cursor-pointer group"
                onClick={() => setSelected(t)}
              >
                <div className="text-4xl text-orange-200 font-serif leading-none mb-3">"</div>
                <p className="text-gray-600 text-sm leading-relaxed flex-1 line-clamp-4">{t.quote}</p>
                <span className="mt-3 text-xs text-orange-500 font-semibold group-hover:underline self-start">
                  Xem chi tiết →
                </span>
          ...
          <button
            onClick={() => go(1)}
            className="absolute -right-4 top-1/2 -translate-y-1/2 z-10 w-10 h-10 bg-white border border-gray-200 rounded-full shadow-md flex items-center justify-center hover:bg-orange-50 hover:border-orange-300 transition-colors"
            aria-label="Tiếp"
          >
            <svg className="w-5 h-5 text-gray-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          {testimonials.map((_, i) => (
            <button
              ...
              className={`rounded-full transition-all duration-300 ${
                i === current ? 'w-6 h-2.5 bg-orange-500' : 'w-2.5 h-2.5 bg-gray-300 hover:bg-gray-400'
              }`}
              aria-label={`Học viên ${i + 1}`}
            />
          ))}
        </div>
